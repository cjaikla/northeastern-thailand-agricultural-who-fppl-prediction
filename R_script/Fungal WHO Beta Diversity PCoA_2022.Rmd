---
title: "Beta diversity PCoA data 2022"
author: "Niar Ibrahim"
...
```{r}
physeq.merge <- physeq.merge %>% microViz::tax_fix()
```

```{r}
# 🔹 Keep only taxa that appear in at least one sample (non-zero total abundance)
...
# ----------------------------
# 1️⃣ CLR Transformation (Aitchison)
# ----------------------------
...
```{r}
# ✅ Fix crop column
# (choose the correct column that exists — likely crop.x or crop.y)
...
```{r}
# ----------------------------
# 4️⃣ PERMANOVA (Aitchison distance)
# ----------------------------
...
```{r}
# ----------------------------
# 5️⃣ PCoA Plot with geom_mark_hull and p-value
# ----------------------------
...
geom_mark_ellipse(
  ...
  ggtitle("PCoA (Aitchison CLR) of WHO FPPL 21 species" ) +
  annotate(
    "text",
...
#Save to jpeg file
```{r warning=FALSE}
jpeg("D:/OneDrive/Dokumen/R/R data/SakonNakhon/Abundance - Distribution/WHOFPPL Beta diversity PCoA-Species-Aitchison SakonNakhon Bell2022.jpeg",units = "in",  width = 6, height = 6, res = 1000)
p_clr
dev.off()
```
