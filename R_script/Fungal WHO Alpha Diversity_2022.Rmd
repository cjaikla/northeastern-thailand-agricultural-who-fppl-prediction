---
title: "Fungi_WHO_Sakon_Alpha_Div"
...
```{r}
sample_data(physeq.merge)$crop <- factor(sample_data(physeq.merge)$crop)
```
...
```{r}
ylabel <- data.frame("ylabel" = c(10, 8, 19, 15)) #LAC surface
...
physeq.plot.richness.crop = plot_richness(physeq.merge, x="crop", color=("crop"), measures=c("Observed")) + 
  geom_boxplot(lwd=0.5) + ggtitle("SakonNakhon 2023 WHO-FPPL Alpha Diversity by crop")  + 
  annotate("text", x = 0.9, y = 25, label = "paste('ANOVA, ', italic('p'), ' = 0.43')", parse = TRUE, size = 3.5) + 
  theme_pubr(border= TRUE, legend = c("none")) + 
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_colour_hue(l = 58) +
...
#Save to tiff file
```{r warning=FALSE}
tiff("WHOFPPL Alpha diversity 2023 CROP.tif",units = "in",  width = 6.5, height = 4.5, res = 1000)
physeq.plot.richness.crop
dev.off()
```
...
#Save to png file
```{r warning=FALSE}
png("WHOFPPL Alpha diversity 2023 CROP.png", units = "in",  width = 6.5, height = 4.5, res = 1000)
physeq.plot.richness.crop
dev.off()
```